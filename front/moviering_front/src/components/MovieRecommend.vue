    ...
    <div class="container">
      ...
      <div class="movie_list_all">
        <div v-for="movie in data">
          <RouterLink
            :to="{ name: 'movie_detail_page', params: { id: movie.pk } }"
          >
            <img
              class="poster_main"
              :src="`https://image.tmdb.org/t/p/w300/${movie.fields.poster_path}`"
              alt="Image"
            />
          </RouterLink>
        </div>
...
const filter_year = function (data) {
  for (const movie of data) {
    ...
    if (myYear === "bf1970" && new_year < 1970) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "1970s" && new_year < 1980) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "1980s" && new_year < 1990) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "1990s" && new_year < 2000) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "2000s" && new_year < 2010) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "2010s" && new_year < 2020) {
      movielist_filtered.value.push(movie);
    } else if (myYear === "af2020" && new_year >= 2020) {
      movielist_filtered.value.push(movie);
    }
  }
};
